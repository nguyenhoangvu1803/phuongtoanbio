...
class Zo_Flickr_Badges_Widget extends WP_Widget {
	...
	**/			
	function __construct() {
		...
		$widget_options = array('classname' => 'widget_flickr', 'description' => __( 'Displays a Flickr photo stream from an ID', 'ori' ) );
		
		// Create the widget
        parent::__construct($this->prefix, __('ZO Flickr Badge', 'ori'), $widget_options, $control_options );
	...
	**/		
	function widget( $args, $instance ) {
		...
		if ( ! empty( $instance['flickr_id'] ) )
			...
			echo '<p>' . __('Please provide an Flickr ID', 'ori') . '</p>';
	...
	**/		
	function form( $instance ) {
		...
		$defaults = array(
			'title'			=> esc_attr__( 'Flickr Widget', 'ori' ),
			'type'			=> 'user',
		...
		$types = array( 
			'user'  => esc_attr__( 'user', 'ori' ),
			'group' => esc_attr__( 'group', 'ori' )
		);
		$sizes = array(
			's' => esc_attr__( 'Standard', 'ori' ),
			't' => esc_attr__( 'Thumbnail', 'ori' ),
			'm' => esc_attr__( 'Medium', 'ori' )
		);
		$displays = array( 
			'latest' => esc_attr__( 'latest', 'ori' ),
			'random' => esc_attr__( 'random', 'ori' )
		);
		...
		<div id="fbw-<?php echo esc_attr($this->id) ; ?>" class="totalControls tabbable tabs-left">
			...
			<ul class="tab-content">
				<li class="tab-pane <?php if ( $instance['tab'][0] ) : ?>active<?php endif; ?>">
					<ul>
						<li>
							<label for="<?php echo esc_attr($this->get_field_id('title')); ?>"><?php _e('Title', 'ori'); ?></label>
							<span class="controlDesc"><?php _e( 'Give the widget title, or leave it empty for no title.', 'ori' ); ?></span>
							<input class="widefat" id="<?php echo esc_attr($this->get_field_id('title')); ?>" name="<?php echo esc_attr($this->get_field_name('title')); ?>" type="text" value="<?php echo esc_attr( $instance['title'] ); ?>" />
						</li>
						<li>
							<label for="<?php echo esc_attr($this->get_field_id('type')); ?>"><?php _e( 'Type', 'ori' ); ?></label>
							<span class="controlDesc"><?php _e( 'The type of images from user or group.', 'ori' ); ?></span>
						...
						<li>
							<label for="<?php echo esc_attr($this->get_field_id('flickr_id')); ?>"><?php _e('Flickr ID', 'ori'); ?></label>
							...
							<span class="controlDesc"><?php printf( __( 'Put the flickr ID here, go to <a href="%s" target="_blank">Flickr NSID Lookup</a> if you don\'t know your ID. Example: 71865026@N00', 'ori' ), esc_url('http://goo.gl/PM6rZ')); ?></span>
						</li>
						<li>
							<label for="<?php echo esc_attr($this->get_field_id('count')); ?>"><?php _e('Number', 'ori'); ?></label>
							<span class="controlDesc"><?php _e( 'Number of images shown from 1 to 10', 'ori' ); ?></span>
						...
						<li>
							<label for="<?php echo esc_attr($this->get_field_id('display')); ?>"><?php _e('Display Method', 'ori'); ?></label>
							<span class="controlDesc"><?php _e( 'Get the image from recent or use random function.', 'ori' ); ?></span>
						...
						<li>
							<label for="<?php echo esc_attr($this->get_field_id('sizes')); ?>"><?php _e( 'Sizes', 'ori' ); ?></label>
							<span class="controlDesc"><?php _e( 'Represents the size of the image', 'ori' ); ?></span>
				...
				<li class="tab-pane <?php if ( $instance['tab'][1] ) : ?>active<?php endif; ?>">
					<ul>
						<li>
							<label for="<?php echo esc_attr($this->get_field_id('intro_text')); ?>"><?php _e( 'Intro Text', 'ori' ); ?></label>
							<span class="controlDesc"><?php _e( 'This option will display addtional text before the widget content and HTML supports.', 'ori' ); ?></span>
						...
						<li>
							<label for="<?php echo esc_attr($this->get_field_id('outro_text')); ?>"><?php _e( 'Outro Text', 'ori' ); ?></label>
							<span class="controlDesc"><?php _e( 'This option will display addtional text after widget and HTML supports.', 'ori' ); ?></span>
						...
						<li>
							<label for="<?php echo esc_attr($this->get_field_id('custom')); ?>"><?php _e( 'Custom Script & Stylesheet', 'ori' ) ; ?></label>
							<span class="controlDesc"><?php _e( 'Use this box for additional widget CSS style of custom javascript. Current widget selector: ', 'ori' ); ?><?php echo '<tt>#' . $this->id . '</tt>'; ?></span>
