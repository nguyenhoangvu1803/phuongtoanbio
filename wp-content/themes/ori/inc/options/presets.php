...
function zo_presets()
{
    return array(
		...
		'link_hover_color',
		'link_active_color',
		'tbar_info_icon_color',
		'menu_color_first_level',
		'menu_color_hover_first_level',
		'menu_color_sub_level',
		'menu_color_hover_sub_level',
		'menu_background_color_hover_sub_level',
		'menu_border_top',
		'menu_border_bottom',
		'main_logo' // Logo
    );
...
function zo_presets_scripts()
{
    if (isset($_REQUEST['page']) && $_REQUEST['page'] == '_options') {
        wp_register_script('zotheme-presets', get_template_directory_uri() . '/inc/options/js/presets.js', 'jquery', '1.0.0', TRUE);
        wp_localize_script('zotheme-presets', 'data_presets', zo_presets());
        wp_enqueue_script('zotheme-presets');
    }
}
