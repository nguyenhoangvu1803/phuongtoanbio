jQuery(document).ready(function($) {
	...
	function zo_set_presets(presets) {
		...
		$.each(presets, function(key, val) {
			...
			}else if(key == 'menu_border_top') {
				$('#smof_data-' + key + ' .wp-color-result' ).css("background-color",val['border-color']);
				$('input[name="smof_data['+ key +'][border-color]"]' ).data('default-color',val['border-color']);
				$('input[name="smof_data['+ key +'][border-color]"]' ).val(val['border-color']);
			}else{
				/* Color Info */
